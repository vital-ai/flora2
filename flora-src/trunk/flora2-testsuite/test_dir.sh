...
# determine if this is flora or ergo
engine=`$FLORA --noprompt --nofeedback --nobanner <<EOF
chatter{off}.
flora_running_as(flora2)@\plg(flrporting) -->> (write(flora2);write(ergo))@\plg.
\halt.
EOF`

ergofiles=`echo *.ergo`
if [ "$ergofiles" = "*.ergo" -o "$engine" = flora2 ]; then
    ergofiles=
fi
