...
/bin/rm -rf $basedir
tar xpzf ./flora2ball.tar.gz
"$curdir/flora2/admin/makeself/makeself.sh" --notemp $basedir flora2.run 'Installing Flora-2' ./flora2/flora2_config.sh
/bin/rm -f ./flora2ball.tar.gz
/bin/rm -rf $basedir
mv flora2.run $curdir
cd $curdir

echo ""
echo "*** The self-extracting archive is in"
echo "***        $curdir/flora2.run"
echo "*** To install using this archive, move flora2.run"
echo "*** to the desired installation directory, then run ./flora2.run"
