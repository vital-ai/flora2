...
\ifdef{\isfloraman}{
\title{
        \ERGOLITE (a.k.a. \FLSYSTEM): User's Manual
        \vspace{0.7cm}\\
...
\ERGOLITE
(also known under its older name \FLSYSTEM)
...
a suitable installation directory, \textbf{DIR} and then execute it:
%% 
\begin{quote}
  \tt
   sh ./flora2.run
\end{quote}
%% 
The above command will create the folder \texttt{Flora-2} with
two subfolders: \texttt{flora2} and \texttt{XSB}.
The script to run \FLSYSTEM is \texttt{\textbf{DIR}/Flora-2/\ENGINENAME/\ENGINERUN}.  
...
own. This is normal. The cause is a Windows misfeature whereby it often
changes file timestamps during unpacking.
...
\textbf{Note}: \FLSYSTEM does not work under the latest versions of Cygwin 
due to a problem with XSB calling external C procedures under Cygwin.
}
{\input{./ergoisms/install}}
...
%%
\begin{alltt}
  \prompt  [\textit{myfile}].        // e.g., ['c:/My Documents/data'].
  \prompt  [url(\textit{myurl})].    // e.g., [url('http://example.com/data')].
\end{alltt}
%%
or 
%%
\begin{alltt}
  \prompt load\{\textit{myfile}\}.     // e.g., load\{'/home/me/proj/kb'\}.
  \prompt load\{url(\textit{myurl})\}.
\end{alltt}
%%
...
For instance, if that directory has the file \texttt{foo.\flrext} then one
can simply type  \texttt{[foo]} instead of
\texttt{['/home/me/foo.\flrext']}
...
If \texttt{\textit{myfile}.\flrext} exists, it is assumed to be a knowledge
base written for \FLSYSTEM. The
system will compile the knowledge base, if necessary, and then load it. The
...
If there is no \texttt{\textit{myfile}.\flrext} file, the file is assumed to contain
...
%%
\begin{alltt}
  \prompt  [\textit{myfile}>{}>\textit{modulename}].
  \prompt  [url(\textit{myurl})>{}>\textit{modulename}].
\end{alltt}
%%
...
system uses the file with the given name without any modification. The {\tt
  .\flrext} suffix denotes a \FLSYSTEM knowledge base, the {\tt .P} suffix indicates that
...
is dealing with a \FLSYSTEM knowledge base and adds the suffix {\tt .\flrext}. If the
...
\item \emph{Base formulas}: Let $\tt O,M,R_{i},X_{i},C,D,T$ be Id-terms.  In
  ...
  \begin{enumerate}
    \item \label{eq-value-atom} $\tt O[M\mvd V]$, $\tt C[|M\mvd V|]$
    \item $\tt O[M\mvd \{V_1,...,V_n\}]$, $\tt C[|M\mvd \{V_1,...,V_n\}|]$
    \item $\tt O[M\Mvd T]$, $\tt C[|M\Mvd T|]$, $\tt C[|M\{L..H\}\Mvd T|]$
    \item $\tt O[V]$, $\tt C[|V|]$
    \item $\tt O[\Mvd T]$, $\tt C[|\Mvd T|]$
    \item $\tt O[~]$, $\tt C[|~|]$
  \end{enumerate}
  ...
  %%
  The expression (3) above is a \emph{signature frame}. It specifies a
  ...
  classes (inherited by subclasses and individual objects). Third,
  ...
  The form (4) is used for Boolean methods. Unlike the methods in (1) and
  (2), Boolean methods can be either true or false: they do not return any
  ...
  \texttt{Mary[age->5]}) and values pertaining to
  the same method and object
  can be grouped into sets (e.g.,
...
\end{verbatim}
  %% 
  \ERGO supports much richer syntax in the rule heads, including
...
sequence can also represent any Unicode character. Such a character is preceded
with a backslash followed by the letters \texttt{x}, \texttt{u},
\texttt{X}, or \texttt{U}  
followed by 1 to 8 hexadecimal digits (0--F) representing the character's
Unicode value. The sequence of digits \textbf{must} be terminated with a
vertical bar, $\mid$.  For example, \texttt{{\bksl}xd|} is the ASCII character
Carriage Return, \texttt{{\bksl}x3A|} represents the semicolon, while
\texttt{{\bksl}u05D0|} is the Hebrew letter Alef. In
...
not applicable & \tt(~) & parentheses; used to change precedence & not applicable & not applicable\\ \hline
not applicable &\tt . & decimal point & not applicable & not applicable \\ \hline
            &\tt .   & object reference & left & binary \\ \cline{2-5}
400         & \tt :~ ::    & class membership and subclass relationships & left & binary \\ \cline{2-5}
600         & \tt - & minus sign & right & unary \\ \cline{2-5}
            & \tt + & plus sign & right & unary \\ \hline
700         & \tt * & multiplication & left & binary \\ \cline{2-5}
700         & \tt ** & power & left & binary \\ \cline{2-5}
            & \tt / & division & left & binary \\ \hline
800         & \tt- ~+ & subtraction and addition & left & binary \\ \cline{2-5}
            &\tt =< & less than or equals to & not applicable & binary \\ \cline{2-5}
            &\tt >= & greater than or equals to & not applicable & binary \\ \cline{2-5}
1000        &\tt =:= & numeric equals-to & not applicable & binary \\ \cline{2-5}
            &\tt ={\bksl}= & unequal to & not applicable & binary \\ \cline{2-5}
            &\tt \bs{}is & arithmetic assignment & not applicable & binary \\ \cline{2-5}
            &\tt = & unification & left & binary \\ \cline{2-5}
            &\tt != or $\backslash$= & disunification & left & binary \\ \cline{2-5}
            &\tt == & identity & left & binary \\ \cline{2-5}
            &\tt =.. & meta decomposition & left & binary \\  \cline{2-5}
            &\tt !== or \texttt{$\backslash$==} & not identical & left & binary \\ \cline{2-5}
            &\tt @< & lexicographical less-than & left & binary \\  \cline{2-5}
            &\tt ?= & identical or not unifiable & left & binary \\ \hline
1200        & \RULELOGNAF & well-founded negation & not applicable & unary \\ \cline{2-5}
            & \RULELOGNEG & explicit negation & not applicable & unary \\ \cline{2-5}
            & \texttt{$\backslash$+} & Prolog-style negation & not applicable & unary \\ \hline
1250        & \texttt{$\sim$} & semantic unification & left & binary \\ \cline{2-5}
            & \texttt{$!\sim$} or \texttt{$\backslash\sim$} & semantic disunification & left & binary \\ \hline
\ifdef{\isfloraman}{}
{
     990    & \texttt{$\mid\mid$} & string/term concatenation & right & binary \\ \hline
     290    &\tt ++~ -\,-~ \&\& & list append/set union/difference/intersection & right & binary \\ \cline{2-5}
\hline
}
\end{tabular}
}
...
\ifdef{\isfloraman}{}
{
  \input{./ergoisms/ergo-is-extension}
}
...
\ifdef{\isfloraman}{}
{
  min,max,sum,avg & 1 & take a list of numbers; return min, max, sum, and
  average
  \\
...
\end{verbatim}
%% 
will succeed. Of course, {\tt ?- skolem{?X}, ?X:\bs{}skolem}   
...
First, it
can be split into many files with separate namespaces. Each
...
clash.  Second, a file can be composed of several files, and these
...
%%
A \emph{\FLSYSTEM module} is an abstraction that allows a large
knowledge base to be split into separate libraries that can be reused in multiple
ways in the same system. Formally, a module is a pair that consists of a
\emph{name} and a \emph{contents}. The name must be an alphanumeric symbol
(the underscore character, {\tt \_}, is also allowed), and the contents
consists of part of
the knowledge base that is typically loaded from some file (but can also
be constructed runtime by inserting facts
into another module).
...
\emph{loaded into a module}. Other parts of the knowledge base can then invoke
...
file can be loaded into any module and the same file can be loaded even
into two different modules at the same time. The same module can be reused
during the same \FLSYSTEM session by loading another file into that module. In
...
and provide useful methods and predicates ({\it e.g.}, I/O) and,
...
enables users to perform common actions using standard names of
...
%%
\FLSYSTEM provides several commands for compiling and loading files into
...
\end{quote}
%%
compiles {\it myfile}.\flrext for the default module {\tt main}.
...
\end{quote}
%%
This command loads the byte code of the program in the file {\tt \textit{myfile}.\flrext} into the default user
...
is used.  If {\tt \textit{myfile}.\flrext} is newer than the compiled byte code (or if
...
\end{quote}
%%
This loads the byte code of the \FLSYSTEM program {\tt \textit{myfile}.\flrext} into the user module named {\tt
...
is used.  If {\tt \textit{myfile}.\flrext} is newer than that compiled byte code (or if
the byte code does not exist), then
the source file is recompiled and then loaded.
...
%%
\begin{alltt}
    ?- ['mykb1.\flrext', mykb2 >{}> foomod].
\end{alltt}
which loads {\tt mykb1.\flrext} into the module {\tt main} and {\tt
  mykb2.\flrext} into the module {\tt foomod}.
...
\texttt{mykb.\flrext}  exists. If so, the file assumed to contain \FLSYSTEM
code and is compiled as such. If \texttt{mykb.\flrext}  is not found, but {\it
...
There is also a four-place version of \texttt{isloaded}:
...
\end{verbatim}
%% 
will bind \texttt{?FullFile} to the absolute version of the file where the
above query occurs.
...
\end{quote}
%%
will \emph{add} the rules and facts contained in the file {\tt mykb.\flrext} into the
...
\index{flora\_query/4 predicate}
%%
...
together and stored in {\tt foo.\flrext}. Note that the file names must be
...
\end{alltt}
%% 
This module loads the contents of the file {\tt mykb.\flrext} into a module
{\tt foo} and then imports that module.  
The importing module itself contains a fact and a query.

Suppose {\tt mykb.\flrext} is as follows:
%% 
\begin{alltt}
  q(?X) :- p(?X).
  p(123).
...
{\tt main} \emph{included} {\tt mykb.\flrext} instead of importing it, i.e.,
...
answer is returned. In contrast, when {\tt mykb.\flrext} is included then
...
%% 
\begin{alltt}
     \$\{\RULELOGNEG a[b->c]@foo\} =.. [negation(neg), \${a[b->c]@main}]
     \$\{\RULELOGNEG p(a,b)@foo\} =.. [negation(neg), \${p(a,b)@main}]
     \$\{\RULELOGNAF a[b->c]@foo\} =.. [negation(naf), \${a[b->c]@main}]
     \$\{\RULELOGNAF p(a,b)@foo\} =.. [negation(naf), \${p(a,b)@main}]
\end{alltt}
%% 
...
\end{verbatim}
%% 
Other connectives, such as \texttt{==>}, \texttt{<==}, \texttt{<==>}, \texttt{if-then-else}, etc., 
...
It should be noted that it makes no
sense to place non-logical operators in the scope of the quantifiers or
...
Quantifiers and $\rnafarr$ have implicit negation in them, so, in order to
...
\texttt{\bs{}if\,$\phi$\,\bs{}then\,$\psi$\,\bs{}else\,$\eta$} is treated for the purpose of
...
Object-specific information is specified using the formulas of
the form
%% 
\begin{verbatim}
    obj[Meth->Val]
...
will \emph{not} apply to the members of that class or its subclasses. For
instance,
%%
\begin{verbatim}
   person[avg_age -> 40].  
...
class frame formulas that use \texttt{->}-style arrows or to Boolean class
frames, and these formulas are inherited to subclasses and class members.
The key difference is that monotonic inheritance is cumulative and
...
file; it should be omitted on the \FLSYSTEM shell.) 
...
established inheritance to be withdrawn and both {\tt
  policy->pacifist} and \texttt{policy->nonpacifist} 
...
hoes in the opposite direction: from class members and subclasses to
...
is not \texttt{attr->1} (i.e., c[|\RULELOGNEG \texttt{attr->1|]} has no logical
justification).
...
\end{verbatim}
%% 
This says that, for the member \texttt{abc} of class \texttt{cl},  
\verb|att->1| is actually true because this information is specified with
higher priority (\texttt{highpriority})  than the default rule
(\texttt{default(cl)}) above, as indicated by the priority fact
\texttt{\bs{}overrides(highpriority,default(cl))}. 

Despite all the above, negative information does play a role in
...
\item[Inheritance:] {\tt inheritance=none}, {\tt inheritance=flogic}
  \texttt{inheritance=monotonic},  where
  {\tt inheritance=flogic} is the default.
\item[Tabling:] \texttt{tabling=reactive} (default),
  \texttt{tabling=passive}, \texttt{tabling=variant} (default), and
...
%%
As mentioned earlier, the {\tt setsemantics} directive accepts three options:
...
%% 
\begin{alltt}
  \bs{}udf foo(?X,?Y) := p(?Z) \if bar(?X,?Y,?Z).
...
function symbols. If this is not done, the things are not unlikely to work
...
\subsection{Transactional (Logical) Updates}
...
\index{primitive!tdelete\{...\}, t\_delete\{...\}}
\index{primitive!tdeleteall\{...\}, t\_deleteall\{...\}}
\index{primitive!terase\{...\}, t\_erase\{...\}}
\index{primitive!teraseall\{...\}, t\_eraseall\{...\}}
\index{tinsert\{...\}, t\__insert\{...} primitives}
\index{tinsertall\{...\}, t\__insertall\{...} primitives}
\index{tdelete\{...\}, t\_delete\{...\} primitives}
\index{tdeleteall\{...\}, t\_deleteall\{...\} primitives}
\index{terase\{...\}, t\_erase\{...\} primitives}
\index{teraseall\{...\}, t\_eraseall\{...\} primitives}
%
...
\ifdef{\isfloraman}{}
{
    \input{./ergoisms/stealth-updates.tex}
}
...
%% 
\begin{itemize}
\item  {\tt agg} is the \emph{aggregate operator};  it can be one of these:
  \texttt{min},
  \texttt{max}, \texttt{count}, \texttt{sum}, \texttt{avg}, \texttt{setof},
  \texttt{bagof}.
\item {\tt ?X} is called the \emph{aggregation variable}
...
\subsection{Evaluation of Aggregares}\label{sec-aggr-eval}
...
is modified by {\tt Action}. Thus, if {\tt Action} has non-transactional updates,
...
\texttt{\bs{}overrides}, one parameterized rule-tag (as we shall see, rule
...
\end{verbatim}
  ...
  inequality $\bs=$ is used.\footnote{
    ...
    and these rules might involve comparisons, arithmetics, etc., and thus
...
constraints in databases and is very common.
...
{\tt \bs{}iri}, (international resource identifier), {\tt \bs{}time}, {\tt \bs{}date},
...
type. For instance, {\tt "2004-12-24"$\widehat{~}\widehat{~}$\bs{}date}, {\tt
...
{\tt \bs{}TiMe} are assumed to be equivalent. In addition, when the type names have
...
permitted, but evaluate to {\bf false} or \texttt{true} depending on the
...
\end{verbatim}
%% 
If at the top level \texttt{?X} in \texttt{?X:\bs{}integer} remains an
unbound variable, the expression should evaluate to \texttt{undefined} and
leave \texttt{?X} unbound. (Currently, though, such an expression evaluates
to \texttt{true}).     
\hfill$\Box$
...
The general constant symbols mentioned above are partitioned into three subcategories:
...
quoted atom. Prefixes can also be defined at run time using a query:
%% 
\begin{verbatim}
  ?- iriprefix{PREFIXNAME = PREFIXIRI}. 
\end{verbatim}
...
%%
\ifdef{\isfloraman}{
 Note: the method \texttt{minus} exists only in \ERGO.
}
...
\begin{itemize}
  ...
  %%
  \ifdef{\isfloraman}{
    Note: the example involving \texttt{minus} will not work in \FLORA ---
    only in \ERGO.
  }
...
\end{itemize}
\ifdef{\isfloraman}{
 Note: the methods \texttt{toDateTime} and \texttt{minus} exist only in \ERGO.
}
...
\textbf{Do not} confuse character lists with symbols: symbols are
\emph{not} lists and have a completely different representation and one
...
in debugging \FLSYSTEM knowledge bases is: \textbf{never
  ignore any kind of warnings} issued by system. This golden rule actually
...
\end{verbatim}
%% 
The {\tt ?Atom} variable must be bound to a base frame, as
...
\index{\texttt{\bs{}dump\_incomplete\_tables\_after\_exception/1}}
%% 
\begin{alltt}
...
\ifdef{\isfloraman}{}
{
  \input{./ergoisms/terminyzer}
  \input{./ergoisms/runtime}
}
...
\index{timer interrupt handler!fail}
  ...
  \item A \FLSYSTEM predicate declared using the \texttt{:- prolog\{...\}} directive.
...
\ifdef{\isfloraman}{}
{
  ...
  \texttt{foobar(?)} or \texttt{foobar} (i.e., one or zero arguments) .
  The argument (if present) must be an unbound variable, which will
  be bound at runtime to the goal being interrupted.
  A Prolog handler can be also specified as \texttt{foobar(?)@\bs{}prolog} or
  \texttt{foobar(?)@\bs{}prolog(\emph{mod})}. The latter is especially useful if
  \texttt{foobar/1} is in the Prolog's module \emph{mod} rather than
  the \texttt{usermod} module. In either of these cases, \texttt{foobar/1} 
  ...
  \texttt{\bs{}@}, if the current module is desired (see
...
during loading. For instance, in either of the above cases
...
\subsubsection{Contriling the Number of Active Goals}
...
\end{quote}
%%
The form \texttt{option=val1+val2+val3} is also supported. For instance,
...
\item {\tt ?Stream[write(?Obj)]}, \texttt{?Stream[writeln(?Obj)]} --- like 
  ...
  stream.
\item {\tt ?Stream[write(?Obj,?Option)]}, \texttt{?Stream[writeln(?Obj,?Option)]} --- like 
  \texttt{write(?Obj,?Option)}, \texttt{writeln(?Obj,?Option)} but write to a
  previously open output {\tt ?Stream} instead of the current output
  stream.
\item {\tt ?Stream[fmt\_write(?Format,?O)]} --- same as
  \texttt{fmt\_write(?Format,?O)}, but uses {\tt ?Stream} for the
  output. 
\item {\tt ?Stream[fmt\_write(?Format,?O,?Options)]} --- same as
...
can be seen as an intermediate devicee. There are three display modes:
\texttt{default}, \texttt{answer},  and \texttt{debug}.  At present, the only difference is
that in the \texttt{debug} mode
the Skolems are written out using their internal representation while in
the default mode the output looks like a \FLSYSTEM term. In the
\texttt{answer} mode the Skolems are shown
...
The primitive \texttt{displaymode\{?X\}} can be used to find out the
...
\end{verbatim}
%% 
while in the default schema, \texttt{flora}, the answer will be shown as
%% 
\begin{verbatim}
  ergo> ?X=p(?p).
...
\end{verbatim}
%% 
The display schema also affects the \texttt{write(...)@\bs{}io} predicates. 

\index{displayschema\{...\} primitive}
\index{primitive!displayschema\{...\}}
The primitive \texttt{displayschema\{?X\}} tells which schema is currently
in effect. 
...
%%reified, then it will be built for the module specified in {\tt ?Mod}. If
...
\end{verbatim}
%%
then {\tt foo.\flrext} will be compiled without the macros and dump the
...
Editing and debugging \FLSYSTEM knowledge bases can be greatly simplified with the
help of \emph{flora-mode}, a special Emacs editing mode designed
specifically for \FLSYSTEM. Flora-mode provides support for syntactic
highlighting, automatic indentation, and the ability to run \FLSYSTEM
...
putting the following command in the file \texttt{\ensuremath{\sim}/.emacs}  (or
\texttt{\ensuremath{\sim}/.xemacs}, if you are running one of the newer versions of XEmacs):
%%
\begin{quote}
  \tt
...
Finally, you must tell X/Emacs how to recognize \FLSYSTEM files, so
Emacs will be able to invoke the Flora major mode automatically when you
are editing such files:
%%
\begin{alltt}
(setq auto-mode-alist (cons \textquotesingle("\bs{}\bs{}.\flrextmath\$" . flora-mode) auto-mode-alist))
\\
(autoload \textquotesingle{}flora-mode "flora" "Major mode for editing Flora-2 knowledge bases." t)
\end{alltt}
%%$
...
\begin{itemize}
  ...
  \item {\sf In XEmacs:} select {\tt Options.Syntax
      ...
      \end{quote}
      %%
      in  \texttt{\ensuremath{\sim}/.emacs} or  \texttt{\ensuremath{\sim}/.xemacs} (whichever is used by your
      XEmacs).
\end{itemize}
%%
...
First, whenever you edit a \FLSYSTEM file, you will see the ``Flora'' menu
...
enables it to provide correct indentation (in many cases).
...
your emacs startup file (\texttt{\ensuremath{\sim}/.emacs} or  \texttt{\ensuremath{\sim}/.xemacs}):
%%
\begin{alltt}
...
\end{alltt}
%%
compiles the knowledge base found in the file {\tt mykb.\flrext} and generates the
following files: {\tt mykb.P}, {\tt mykb\_main.\ofile}, and {\tt
  mykb.fdb} (if {\tt mykb.\flrext} contains facts in addition to rules).
By default, {\tt load\{mykb\}} loads the knowledge base into the default user
module named {\tt main}. If {\tt mykb.\flrext} contains \fl facts, all these
...
code {\tt mykb.\ofile}, which is then renamed to {\tt
  ...
  mykb.\flrext} contains queries, they are immediately executed by Prolog
